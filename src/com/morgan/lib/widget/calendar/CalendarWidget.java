...
public class CalendarWidget extends LinearLayout {
    ...
    private void initView() {
        LayoutInflater.from(mContext).inflate(R.layout.lib_calendar_view, this);
        ...
        mSlideLeftIn = AnimationUtils.loadAnimation(mContext, R.anim.lib_slide_left_in);
        mSlideLeftOut = AnimationUtils.loadAnimation(mContext, R.anim.lib_slide_left_out);
        mSlideRightIn = AnimationUtils.loadAnimation(mContext, R.anim.lib_slide_right_in);
        mSlideRightOut = AnimationUtils.loadAnimation(mContext, R.anim.lib_slide_right_out);
    ...
    private void updateGirdView() {
        ...
        int indexOfLastDay = indexOfFirstDay + getDaysBetweenDate(firstDayOfMonth, lastDayOfMonth);

        mLastMonthView.setListDay(initList(firstDayOfMonth, indexOfFirstDay), indexOfFirstDay, indexOfLastDay);
        ...
        indexOfLastDay = indexOfFirstDay + getDaysBetweenDate(firstDayOfMonth, lastDayOfMonth);

        mNextMonthView.setListDay(initList(firstDayOfMonth, indexOfFirstDay), indexOfFirstDay, indexOfLastDay);
        ...
        mCurrentMonthView.setOnItemClickListener(new OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position < mIndexOfFirstDay) {
                    showPrePage();
                    int preMonthPosition = mLastMonthView.getCurrentMonthPos(position);
    ...
    private ArrayList<Day> initList(Date firstDayOfMonth, int indexOfFirstDay) {
        ...
        if (indexOfFirstDay > 0) {
            for (int i = 0; i < indexOfFirstDay; i++) {
                Day day = new Day(firstDayOfMonth.getTime() - (indexOfFirstDay - i) * 86400000);
                days.add(day);
            }
        }
        // 填上剩下的所有日期
    ...
    private void initList() {
        ...
        mIndexOfLastDay = mIndexOfFirstDay + getDaysBetweenDate(firstDayOfMonth, lastDayOfMonth);
    ...
    class GestureListener extends SimpleOnGestureListener {
        ...
        public boolean onSingleTapUp(MotionEvent e) {
            mCurrentMonthView.selectPosition((int) e.getX(), (int) e.getY());
            return false;
        }
